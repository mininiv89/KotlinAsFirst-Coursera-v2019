...
fun bestHighJump(jumps: String): Int {
    ...
    var res: Int = 0
    return try {
        if (str.size % 2 != 0) throw Exception("Неправильный формат строки")
        ...
        for (i in 1 until str.size step 2) {
            ...
            str[i].map {
                ...
                } else throw Exception("Не правильный ввод")
            }
...
fun plusMinus(expression: String): Int {
    ...
    try {
        for (i in str.indices step 2) {
            for (element in str[i]) element.toString().toInt()
        }
        result += str[0].toInt()
...
fun firstDuplicateIndex(str: String): Int {
    ...
    if (list.size > 1) {
        for (i in 1 until list.size) {
            if (list[i - 1].toLowerCase() == list[i].toLowerCase()) return str.indexOf(list[i - 1] + " " + list[i])
        }
    }
    return -1
}
